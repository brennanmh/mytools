...
def update_dns_record(zone_id, record_id, ip, email, api_key, dry_run=False):
    ...
    if dry_run:
        ...
        logging.info(
            f"Dry run: would send PUT request to {url} with data {data} and headers {headers}"
        )
        return {"success": True}
    logging.debug(
        f"Sending PUT request to {url} with data {data} and headers {headers}"
    )
...
def get_credentials(config_file=None):
    ...
    if config_file:
        ...
        if not email or not api_key:
            raise Exception(
                "CLOUDFLARE_EMAIL and CLOUDFLARE_API_KEY environment variables must be set"
            )
        logging.debug(f"Retrieved email: {email}")
        logging.debug(f"Retrieved API key: {api_key}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Update Cloudflare DNS record for nthroot.org"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        if has_record_changed(ZONE_ID, RECORD_ID, wan_ip, EMAIL, API_KEY):
            ...
            result = update_dns_record(
                ZONE_ID, RECORD_ID, wan_ip, EMAIL, API_KEY, dry_run=args.dry_run
            )
            if args.dry_run:
